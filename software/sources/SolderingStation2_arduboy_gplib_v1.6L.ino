...
void MainScreen() {
  ...
  if (MainScrType) {
    ...
    //温度条
    //框
    arduboy.drawRect(1, 49, 102, 14, 0);
    //条
    if (ShowTemp <= 500) arduboy.fillRect(1, 49, map(ShowTemp, 0, 500, 2, 102), 14, 0);
    //指示部分
    arduboy.fillRect(map(Setpoint, 0, 500, 2, 102) - 2, 50, 3, 13, 0);
    //先来个白底
    arduboy.fillRect(map(Setpoint, 0, 500, 2, 102) - 3, 60, 5, 4, 0);
    arduboy.drawLine(map(Setpoint, 0, 500, 2, 102) - 1, 60, map(Setpoint, 0, 500, 2, 102) - 1, 49);

    //画指示针
    arduboy.drawSlowXYBitmap(map(Setpoint, 0, 500, 2, 102) - 3, 59, Pointer, 5, 4, 1);

    //功率条
    arduboy.drawRect(104, 49, 23, 14, 0);
    arduboy.fillRect(104, 49, map(Output, 255, 0, 0, 23), 14, 0);
...
uint8_t MenuScreen(uint8_t selected) {
  ...
  do {
    ...
    arduboy.drawSlowXYBitmap(48, 48, Txt_table[selected + QueryMenuObject()], 36, 16, 1);
    arduboy.display();
    CheckLastButton();
